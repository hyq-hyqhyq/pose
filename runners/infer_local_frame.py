...
# 渲染函数
def lookat_cam(scene, center, radius, az_deg, el_deg, vfov_deg, W, H, near=0.01):
    ...
    cam = scene.camera
    cam.set_projection(vfov_deg, W/H, near, 1000.0,
                       o3d.visualization.rendering.Camera.FovType.Vertical)
    cam.look_at(center, eye, up)
    return cam.get_view_matrix(), cam.get_projection_matrix()
...
def render_rgbd(obj_path, out_dir, i):
    """
    渲染RGB-D图像的独立函数
    """
    ap = argparse.ArgumentParser()
    ap.add_argument("--obj", default=obj_path)
    ap.add_argument("--out_dir", default=out_dir)
    ap.add_argument("--width", type=int, default=640)
    ap.add_argument("--height", type=int, default=480)
    ap.add_argument("--vfov", type=float, default=30, help="vertical field of view in degrees")
    ap.add_argument("--category", type=str, default="knife")
    ap.add_argument("--azimuths", type=int, default=8, help="views around yaw")
    ap.add_argument("--index", type=int, help="index of the view")
    ap.add_argument("--elevations", type=str, default="0", help="comma list deg")
    ap.add_argument("--radius_scale", type=float, default=1, help="camera distance factor")
    args = ap.parse_args([])  # 空列表表示不使用命令行参数

    os.makedirs(args.out_dir, exist_ok=True)
    mesh = o3d.io.read_triangle_mesh(args.obj)
    if mesh.is_empty():
        raise RuntimeError(f"Failed to load {args.obj}")
    ...
    scene.set_background(np.array([1,1,1,1], dtype=np.float32))
    mat = o3d.visualization.rendering.MaterialRecord(); mat.shader="defaultLit"
    scene.add_geometry("obj", mesh, mat)

    # bbox / camera distance
    aabb = mesh.get_axis_aligned_bounding_box()
    center = aabb.get_center()
    extent = aabb.get_extent()
    radius = 1 * max(extent)
    vfov = math.radians(args.vfov)
    dist = (radius / math.tan(vfov/2.0)) * args.radius_scale
    cat = args.category

    az_list = list(np.linspace(0, 360, args.azimuths, endpoint=False))
    az_list = [az_list[i]]
    el_list = [float(x) for x in args.elevations.split(",") if x!=""]

    idx = 1
    for el in el_list:
        for az in az_list:
            view, proj = lookat_cam(scene, center, dist, az, el, args.vfov, W, H)
            img_o3d   = renderer.render_to_image()
            depth_o3d = renderer.render_to_depth_image(z_in_view_space=True)
            color = np.asarray(img_o3d)[:, :, :3][:, :, ::-1]   # BGR for cv2.imwrite
            depth = np.asarray(depth_o3d).astype(np.float32)
            valid = np.isfinite(depth) & (depth > 0)
            mask = (~valid).astype(np.float32)
            prefix = os.path.join(args.out_dir, f"{1:06d}_")
            save_exr(prefix+"depth.exr" ,depth)
            save_exr(prefix+"mask.exr" ,mask)
            cv2.imwrite(prefix+"color.png", color)

            # 按当前视场计算的针孔相机内参
            fy = (H/2.0) / math.tan(vfov/2.0)
            fx = (W/2.0) / math.tan(vfov/2.0)
            cx, cy = W/2.0, H/2.0
            ...
            print(f"[{idx}] az={az:.1f}, el={el:.1f} → {prefix}*")
            idx += 1
...
def save_mesh_pose_normalized(mesh_in, mesh_out, poses):

    # ---------- 读入网格 ----------
    mesh = o3d.io.read_triangle_mesh(mesh_in)
    if mesh.is_empty():
        raise RuntimeError(f"Failed to load mesh: {mesh_in}")
    mesh.compute_vertex_normals()
    ...
    for p in poses:
        ...
        P = P.reshape(-1, *(4, 4))[0]
        if P.shape != (4, 4):
            raise ValueError(f"P_avg must be (4,4); got {P.shape}")
        if P.shape == (4, 4):
            M = P[:3, :3]
            Ms.append(M)
        elif P.shape == (3, 3):
            ...
            raise ValueError("P_avg must be 3x3 or 4x4")

    from scipy.spatial.transform import Rotation as R
    angles = []
    index = 0
    for i in [2, 3, 5, 6]:
        rotation = R.from_matrix(Ms[index])
        index += 1
        euler_angles = rotation.as_euler('yzx', degrees=True)
        print(f"Euler angles (Y, Z, X): {euler_angles}")
        yaw_deg = euler_angles[0] - 45 * (i - 2)
        if yaw_deg >= 0:
            yaw_deg -= 360
        print(f"Yaw angle (frame {i}): {yaw_deg}")
        angles.append(yaw_deg)

    yaw_deg = sum(angles) / len(angles)
    # 规范范围到 [-180,180)
    if yaw_deg >= 180.0: yaw_deg -= 360.0
    if yaw_deg <  -180.0: yaw_deg += 360.0

    # ---------- 量化到 0/90/180/270 ----------
    k = int(round(yaw_deg / 90.0))
    print(yaw_deg, k)
    snap_deg = (k * 90.0) % 360.0     # 0/90/180/270

    # 以 AABB 中心为旋转中心，避免绕远点引入偏移
    aabb = mesh.get_axis_aligned_bounding_box()
    center_before = aabb.get_center()

    th = math.radians((-snap_deg) % 360.0)
    c, s = math.cos(th), math.sin(th)
    Ry = np.array([[ c, 0, s],
                   [ 0, 1, 0],
                   [-s, 0, c]], dtype=float)

    mesh.rotate(Ry, center=center_before)

    # ---------- 把网格平移到原点 ----------
    aabb_after = mesh.get_axis_aligned_bounding_box()
    center_after = aabb_after.get_center()
    mesh.translate(-center_after)

    # ---------- 保存 ----------
    ok = o3d.io.write_triangle_mesh(mesh_out, mesh)
    if not ok:
        raise RuntimeError(f"Failed to save mesh: {mesh_out}")

    info = {
        "yaw_deg_estimated": float(yaw_deg),
        "snap_deg": float(snap_deg),
        "applied_deg": float((-snap_deg) % 360.0),
        "recentered_to": [0.0, 0.0, 0.0]
    }
    print(f"[normalized] yaw≈{yaw_deg:.2f}°, snap={snap_deg:.0f}°, applied={info['applied_deg']:.0f}°, recentered→(0,0,0)")
    return info


def main():
    ######################################## PARAMETERS ########################################
    DATA_PATH = 'output'                 # Path to the data
    TRACKING = False                                           # Tracking mode
    ...
    MESH_IN  = 'object3.obj'
    MESH_OUT = 'mesh_pose_normalized.obj'
    ######################################## PARAMETERS ########################################
    import json
    ...
    for index, color_image in enumerate(tqdm(color_images)):
        ...
        color_image_w_pose = visualize_pose(data, pose, length, visualize_image=True)
    ...
    for i in [2, 3, 5, 6]:
        ...
        render_rgbd("object3.obj", "output", i)
        color_images = sorted(glob.glob(DATA_PATH + '/*_color.png'))
    ...
    if os.path.exists(MESH_IN):
        save_mesh_pose_normalized(MESH_IN, MESH_OUT, poses)
        print(f"[OK] pose-normalized mesh saved to: {MESH_OUT}")
    else:
        print(f"[WARN] MESH_IN not found: {MESH_IN} (skip mesh normalization)")
